 ...
 * The places listing.
 *
 * Structurally the cuisine listing: the same controls, the same cards, the same
 * URL-held state. It differs in one field — the filter pills select a kind of
 * site (`placeTypeId`) — and reuses the `type` URL key that cuisine and works
 * already use for a classification that is not a period.
 */
export function PlacesListing({
  items,
  filters,
  ui,
  children,
  ...
  const results = useMemo(
    () => filterItems(items, query, [{ selected, keyOf: (item) => item.placeTypeId }]),
  ...
  return (
    <>
      <ListingControls
        ...
        searchLabel={ui.listing.places.searchLabel}
        placeholder={ui.listing.places.searchPlaceholder}
        query={query}
      ...
      <div className="mt-14 md:mt-16">
        <SectionHeading
          eyebrow={ui.listing.places.allEyebrow}
          title={ui.listing.places.allTitle}
          description={ui.listing.places.allDescription}
        />
        {results.length > 0 ? (

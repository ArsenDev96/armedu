...
export function generateStaticParams() {
  return articleStaticParams("cuisine", SUPPORTED_LOCALES);
}
...
export async function generateMetadata({ params }: Params): Promise<Metadata> {
  ...
  return articleMetadata(locale, "cuisine", slug);
}

export default async function CuisineArticlePage({ params }: Params) {
  const { locale: raw, slug } = await params;
  const locale = resolveLocale(raw);
  return <ArticleRoute locale={locale} category="cuisine" slug={slug} />;
}

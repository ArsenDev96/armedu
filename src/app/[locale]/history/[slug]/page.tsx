...
export function generateStaticParams() {
  return articleStaticParams("history", SUPPORTED_LOCALES);
}
...
export async function generateMetadata({ params }: Params): Promise<Metadata> {
  ...
  return articleMetadata(locale, "history", slug);
}

export default async function HistoryArticlePage({ params }: Params) {
  const { locale: raw, slug } = await params;
  const locale = resolveLocale(raw);
  return <ArticleRoute locale={locale} category="history" slug={slug} />;
}

...
import { SUPPORTED_LOCALES } from "@/data/types";
import {
  ArticleRoute,
  articleMetadata,
  articleStaticParams,
...
export function generateStaticParams() {
  return articleStaticParams("places", SUPPORTED_LOCALES);
}
...
export async function generateMetadata({ params }: Params): Promise<Metadata> {
  ...
  return articleMetadata(locale, "places", slug);
}

export default async function PlaceArticlePage({ params }: Params) {
  const { locale: raw, slug } = await params;
  const locale = resolveLocale(raw);
  return <ArticleRoute locale={locale} category="places" slug={slug} />;
}

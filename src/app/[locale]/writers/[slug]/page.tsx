...
export function generateStaticParams() {
  return articleStaticParams("writers", SUPPORTED_LOCALES);
}
...
export async function generateMetadata({ params }: Params): Promise<Metadata> {
  ...
  return articleMetadata(locale, "writers", slug);
}

export default async function WriterArticlePage({ params }: Params) {
  const { locale: raw, slug } = await params;
  const locale = resolveLocale(raw);
  return <ArticleRoute locale={locale} category="writers" slug={slug} />;
}
